      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <h3 className="text-3xl md:text-4xl font-bold mb-6">More Than Just a <br /> Courier Service</h3>
            <p className="text-slate-400 text-lg mb-8 leading-relaxed">
              We are an extension of your healthcare facility. Our drivers are trained to handle bio-hazardous materials
              and understand the critical nature of medical deliveries.
            </p>

            <div className="space-y-6">
              {[
                { icon: <Clock4 className="h-6 w-6 text-brand-400" />, title: 'Real-Time Tracking', desc: 'Watch your delivery from pickup to drop-off with our advanced GPS portal.' },
                { icon: <Lock className="h-6 w-6 text-brand-400" />, title: 'HIPAA Compliant', desc: 'Strict adherence to patient privacy laws and secure chain-of-custody protocols.' },
                { icon: <UserCheck className="h-6 w-6 text-brand-400" />, title: 'Certified Drivers', desc: 'OSHA & Bloodborne Pathogen trained couriers, uniformed and ID badged.' },
              ].map((feature, idx) => (
                <div key={idx} className="flex gap-4">
                  <div className="flex-shrink-0 mt-1">
                    <div className="p-2 bg-slate-800 rounded-lg border border-slate-700">{feature.icon}</div>
                  </div>
                  <div>
          ...
          <div className="relative">
            <div className="absolute -inset-4 bg-brand-500/20 blur-2xl rounded-full" />
            <img
